...
function countSameElements(collection) {
  ...
  //第2步
  var arrObjectCount = []; //创建一个专门放置用来统计summary的对象的数组
  //根据["a", "e", "h", "t", "f", "c", "g", "b", "d"]里的字母，
  //把类似于{name: 'a', summary: 0}的对象写进arrObjectCount里面
  for (var i = 0; i < temporaryArray.length; i++) {
    arrObjectCount.push({
      name: temporaryArray[i],
      summary: 0
    });
  }
  ...
  for (var i = 0; i < arrObjectCount.length; i++) {
    ...
    for (var j = 0; j < collection.length; j++) {
      if (arrObjectCount[i].name == collection[j]) {
        number++;
        continue;
      } else if (arrObjectCount[i].name == collection[j].slice(0, 1)) {
        number += Number(collection[j].match(/\d+/g));
      }
    }
    arrObjectCount[i].summary = number;
  }
