...
function createUpdatedCollection(collectionA, objectB) {
  ...
  for (var i = 0; i < collectionA.length; i++) {
    ...
    //否则把当前项push到临时数组里面
    if (temporaryArray.indexOf(collectionA[i].slice(0, 1)) == -1) {
      temporaryArray.push(collectionA[i].slice(0, 1));
    }
  }
  ...
  for (var i = 0; i < arrObjectCount.length; i++) {
    ...
    for (var j = 0; j < collectionA.length; j++) {
      if (arrObjectCount[i].key == collectionA[j]) {
        number++;
        continue;
      } else if (arrObjectCount[i].key == collectionA[j].slice(0, 1)) {
        number += Number(collectionA[j].match(/\d+/g));
      }
    }
